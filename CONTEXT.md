...
**Judge**:
唯一保留的 spawn 子 agent；关键产出的独立质量门。历史上面向 benchmark 评分，现 benchmark 评分已迁至 fork 的确定性 `custom_check`（见下）；judge agent 配置的删除属 ADR-0002 收尾批次，fork CI 验证前勿动 `openclaw.json`/`allowAgents`/`judge/`。
...
**custom_check**:
确定性评分脚本（`custom_checks/*.py`），从 agent workspace 产物取分，**不依赖 judge agent**。fork 用它取代旧自搓 harness 的 spawn-judge 评分。
...
_Avoid_: judge（benchmark 评分已由 fork 的 `custom_check` 取代；judge agent 配置的删除属 ADR-0002 收尾批次，fork CI 验证前勿动 `openclaw.json`/`allowAgents`/`judge/`）、临时 agent（fork 不再新建）。
