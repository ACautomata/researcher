...
description: Generate an AutoResearch project on demand from a user brief, research idea, RTS specification, or reference archive. Use when the user asks the autoresearch agent to scaffold, update, or validate an AutoResearch project. This skill defines the agent workflow only; do not commit generated project source code into the OpenClaw configuration repository.
---
