...
- CI 在既有 `openclaw-bench` 容器内跑 `run.py`（state=`/home/node/.openclaw`=main agent 真实 state；`_uses_isolated_state()=False`，三个 seed 函数全 no-op，零污染--仅指 config/auth seed 不污染，不含 trial 间 workspace 状态）。
