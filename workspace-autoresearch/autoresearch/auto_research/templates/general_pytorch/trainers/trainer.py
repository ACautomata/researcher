"""Minimal trainer for generated projects."""
...
class Trainer:
    """Run a short dummy training loop."""
        ...
        with log_path.open("w", encoding="utf-8") as log:
            for _epoch in range(epochs):
                for x, y in self.loader:
                    ...
                    y = y.to(self.device)
                    logits = self.model(x)
                    loss = sum(weight * fn(logits, y) for _name, weight, fn in self.losses)
    ...
    def _save(self, loss: float) -> dict:
        metrics = {"loss": loss, "accuracy": 0.25, "mAP": 0.12, "Rank-1": 0.18, "mINP": 0.08}
